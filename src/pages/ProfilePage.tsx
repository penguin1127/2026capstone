...
const ASSETS = [
  { title: 'Potion Icons', rating: '4.7', price: 0 },
  { title: 'RPG UI Kit', rating: '4.9', price: 5900 },
  { title: 'Gem & Currency', rating: '4.8', price: 0 },
]
    ...
    <div style={{ background: '#0d1117', color: '#e6edf3' }}>
      ...
      <div className="relative h-48 overflow-hidden"
        ...
        <div className="absolute inset-0 opacity-20"
          style={{
            backgroundImage: 'repeating-linear-gradient(0deg,transparent,transparent 20px,rgba(255,255,255,0.2) 20px,rgba(255,255,255,0.2) 21px), repeating-linear-gradient(90deg,transparent,transparent 20px,rgba(255,255,255,0.2) 20px,rgba(255,255,255,0.2) 21px)'
          }} />
        ...
        <div className="max-w-screen-2xl mx-auto px-6">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 pt-2" style={{ marginTop: -32 }}>
            ...
            <div className="flex items-end gap-4">
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center font-bold text-2xl border-4 shadow-xl flex-shrink-0"
          ...
          {/* 탭 */}
          <div className="flex gap-0 border-t" style={{ borderColor: '#30363d' }}>
            {[['works', '작품'], ['assets', '에셋'], ['liked', '좋아요'], ['following', '팔로잉']].map(([key, label]) => (
              <button key={key} onClick={() => setTab(key)}
                className="px-5 py-3 font-bold text-sm border-b-2 transition-colors"
                style={tab === key
                  ? { color: '#2f81f7', borderColor: '#2f81f7' }
      ...
      {/* 탭 콘텐츠 */}
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 pt-4 pb-20">

        {/* 작품 탭 — Pixiv 스타일 dense grid */}
        {tab === 'works' && (
          <div>
                ...
                <Link key={work.id} to={`/gallery/${work.id}`}
                  ...
                  {work.featured && (
                    <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold"
                      style={{ background: '#2f81f7', color: '#fff' }}>Featured</div>
                  )}
                </Link>
              ))}
            </div>
        ...
        {tab === 'assets' && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-2">
            {ASSETS.map(asset => (
              <div key={asset.title} className="rounded-xl border overflow-hidden cursor-pointer group hover:shadow-md transition-all"
                style={{ background: '#21262d', borderColor: '#30363d' }}>
                <div className="aspect-square checkerboard flex items-center justify-center text-4xl group-hover:opacity-80 transition-opacity" />
                <div className="p-3">
        ...
        {/* 좋아요 탭 */}
        {tab === 'liked' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8 }} className="mt-2">
            {LIKED.map(item => (
        ...
        {tab === 'following' && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-2">
            {FOLLOWING.map(user => (
              <div key={user.name} className="rounded-xl border p-4 text-center cursor-pointer hover:shadow-md transition-all"
                style={{ background: '#21262d', borderColor: '#30363d' }}>
                ...
                <button className="px-4 py-1 rounded-full text-xs font-bold transition-all hover:bg-[#2f81f7] hover:text-white hover:border-[#2f81f7]"
